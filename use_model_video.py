...
while cap.isOpened():
    ...
    cv2.rectangle(img, (best_loc[0], best_loc[1]), (best_loc[0]+box_size, best_loc[1]+box_size), (255, 0, 0), 2)
    print(final_prediciotn)
    for numit in final_prediciotn:
        final_pred += str(numit)
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(img, final_pred,  (best_loc[0], best_loc[1]-5), font, 1, (255, 50, 0), 2, cv2.LINE_AA)
    # cv2.imshow(f, img)
    cv2.imwrite('video_output_imgs/'+str(count)+'.jpg', img)
    count += 1
    print('finished frame {}'.format(str(count)))
cap.release()
